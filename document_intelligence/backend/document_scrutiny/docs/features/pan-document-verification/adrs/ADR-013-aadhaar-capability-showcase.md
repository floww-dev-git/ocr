...
- wrong-person → the cross-checks fail;
...
- New Aadhaar capabilities, each a real check with a stable id: `:aadhaar-format`
  (now also a checksum verdict), `:qr-consistency`, `:scan-quality` — all generated
  by the existing engine, streamed and rendered with no new UI plumbing.
...
- A deterministic specimen generator (`scripts/generate_aadhaar_specimens.py`,
  segno-generated QR, watermarked "SPECIMEN — NOT A REAL AADHAAR", no PII) and a
  read-only demo endpoint serving them, driven by an in-app scenario picker.
